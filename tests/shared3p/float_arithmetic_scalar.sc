...
bool test_addition_random(T data){
    ...
    for(int i = -1; i < 2;++i){
        for(uint j = 0; j < 6; ++j){
            ...
            if (!isRelativeErrorSmall(c, control[cur_pos])) {
                result = false;
                break;
            }
            c = b + a;
...
bool test_division_random(T data){
    T[[1]] in = {1,1000,1000000,-1,-1000,-1000000};
    pd_shared3p T[[1]] control = {-1,-0.001,-0.000001,1,0.001,0.000001,0,0,0,0,0,0,1,0.001,0.000001,-1,-0.001,-0.000001};
    bool result = true;
    uint temp = 0;
    uint cur_pos = 0;
    pd_shared3p T a;pd_shared3p T b;pd_shared3p T c;
...
void main(){
    string test_prefix = "Addition with two private values";
    {
        ...
        test(test_prefix, isRelativeErrorSmall(a+b, c), c);
    }
    {
    ...
    {
        pd_shared3p float32 a = 15; float32 b = 174; pd_shared3p float32 c = 189;
        test(test_prefix, isRelativeErrorSmall(b+a, c), c);
    }
    {
        pd_shared3p float64 a = 175; float64 b = 45876; pd_shared3p float64 c = 46051;
        test(test_prefix, isRelativeErrorSmall(b+a, c), c);
