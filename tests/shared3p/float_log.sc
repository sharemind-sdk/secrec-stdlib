...
template<type T>
PrecisionTest<T> test_log(T data){
    ...
    pd_shared3p T[[1]] base(20) = 10;
    pd_shared3p T[[1]] c = log(a, base);
...
void main(){
    ...
    {
        PrecisionTest<float32> rv = test_log(0::float32);
        test(test_prefix, rv);
    }
    {
        PrecisionTest<float64> rv = test_log(0::float64);
        test(test_prefix, rv);
    }
